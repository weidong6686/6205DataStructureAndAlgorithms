...
public class Hauffman {
	...
	private void makeTree() {		
		...
		if (size == 1) {
			...
			sumNode.count = 1;
			sumNode.num = i;
			sumNode.left = priorityQueue.poll();
			priorityQueue.add(sumNode);
			System.out.println("Internal node " + i + " : Left " + 
					sumNode.left.ch + "(" + sumNode.left.count + ") Right " +
					"null" + " Weight = " + sumNode.count);				
		}
...
//				if (node.right == null) { //once only one leaf and one interval
//					fw.write(" \"" + node.num + "\\n" + node.count + "\"\r\n");
//					break;
//				}		
				// nodes in the queue must have a left child			
				temp = node.left;
				fw.write(" \"" + node.num + "\\n" + node.count + "\" ->\"");				
				fw.write(temp.num + "\\n" + temp.count);
				...
				fw.write("\" [color=red]\r\n");
				//unless only one letter, there should be a right child
				temp = node.right;
				if (temp != null) {
					fw.write(" \"" + node.num + "\\n" + node.count + "\" ->\"");				
					fw.write(temp.num + "\\n" + temp.count);
